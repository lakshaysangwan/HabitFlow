...
describe('checkRateLimit', () => {
  ...
  it('blocks the (limit + 1)th request', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      await checkRateLimit(db, key, { limit: 5, windowSeconds: 60 })
    }
  ...
  it('different keys do not interfere', async () => {
    ...
    // Max out key A
    for (let i = 0; i < 3; i++) {
      await checkRateLimit(db, keyA, { limit: 3, windowSeconds: 60 })
    }
    expect(await checkRateLimit(db, keyA, { limit: 3, windowSeconds: 60 })).toBe(false)

    // Key B should still be free
    expect(await checkRateLimit(db, keyB, { limit: 3, windowSeconds: 60 })).toBe(true)
  })
  ...
  it('resets count when window expires', async () => {
    ...
    for (let i = 0; i < 2; i++) {
      await checkRateLimit(db, key, { limit: 2, windowSeconds: 1 })
    }
    expect(await checkRateLimit(db, key, { limit: 2, windowSeconds: 1 })).toBe(false)
    ...
    expect(await checkRateLimit(db, key, { limit: 2, windowSeconds: 1 })).toBe(true)
  })

  it('first request for a new key is always allowed', async () => {
    const db = getDB(env.DB)
    const key = `brand-new-${Date.now()}-${Math.random()}`
    expect(await checkRateLimit(db, key, { limit: 1, windowSeconds: 60 })).toBe(true)
  })
})
